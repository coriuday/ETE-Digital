...
async def get_platform_stats(
    ...
    return PlatformStats(
        ...
        total_employers=total_employers, total_jobs=total_jobs,
...
async def list_all_jobs(
    ...
    return AdminJobListResponse(
        jobs=[
            AdminJobResponse(
                id=str(j.id), title=j.title, company=j.company or "",
                employer_id=str(j.employer_id), status=j.status.value,
                job_type=j.job_type.value, location=j.location,
                applications_count=j.applications_count or 0,
                views_count=j.views_count or 0, created_at=j.created_at
            )
            for j in jobs
        ],
        total=total
...
async def list_all_applications(
    ...
    profiles_map = {p.user_id: p for p in (await db.execute(select(UserProfile).where(UserProfile.user_id.in_(candidate_ids)))).scalars().all()}
    ...
    return AdminApplicationListResponse(
        applications=[
            AdminApplicationResponse(
                ...
                status=a.status.value, match_score=a.match_score, created_at=a.created_at
            )
