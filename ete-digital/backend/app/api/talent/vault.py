...
def item_to_response(item):
    return VaultItemResponse(
        id=str(item.id), candidate_id=str(item.candidate_id), type=item.type,
        title=item.title, description=item.description, file_url=item.file_url,
        file_type=item.file_type, file_size_bytes=item.file_size_bytes,
        item_metadata=item.item_metadata, is_verified=item.is_verified,
        verified_by=item.verified_by,
        tryout_submission_id=str(item.tryout_submission_id) if item.tryout_submission_id else None,
        share_count=item.share_count, view_count=item.view_count,
...
def token_to_response(token, frontend_url):
    return ShareTokenResponse(
        id=str(token.id), vault_item_id=str(token.vault_item_id),
        token=str(token.token),
        share_url=f"{frontend_url}/vault/shared/{token.token}",
        expires_at=token.expires_at, max_views=token.max_views,
        view_count=token.view_count, is_revoked=token.is_revoked,
        shared_with_email=token.shared_with_email,
        shared_with_company=token.shared_with_company,
        created_at=token.created_at, last_accessed_at=token.last_accessed_at
    )
...
async def get_my_vault_items(
    ...
    items, total = await vault_service.get_candidate_vault(
        db=db, candidate_id=uuid.UUID(current_user["user_id"]),
        page=page, page_size=page_size, item_type=item_type
    )
    return VaultItemListResponse(items=[item_to_response(i) for i in items], total=total, page=page, page_size=page_size)
...
async def update_vault_item(
    ...
    item = await vault_service.update_vault_item(
        db=db, item_id=uuid.UUID(item_id),
...
async def delete_vault_item(
    ...
    await vault_service.delete_vault_item(
        db=db, item_id=uuid.UUID(item_id), candidate_id=uuid.UUID(current_user["user_id"])
    )
...
async def create_share_tokens(
    ...
    tokens = await share_token_service.create_share_tokens(
        db=db, candidate_id=uuid.UUID(current_user["user_id"]),
        vault_item_ids=vault_item_ids, expires_hours=share_data.expires_hours,
        max_views=share_data.max_views, shared_with_email=share_data.shared_with_email,
...
async def revoke_share_token(
    ...
    await share_token_service.revoke_share_token(
        db=db, token_id=uuid.UUID(token_id), candidate_id=uuid.UUID(current_user["user_id"])
    )
...
async def access_shared_vault(token: str, db: AsyncSession = Depends(get_db)):
    ...
    return VaultAccessResponse(item=item_to_response(item), remaining_views=remaining_views, expires_at=share_token.expires_at)
