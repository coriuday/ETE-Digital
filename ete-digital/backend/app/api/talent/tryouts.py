...
def tryout_to_response(tryout):
    return TryoutResponse(
        id=str(tryout.id), job_id=str(tryout.job_id), title=tryout.title,
        description=tryout.description, requirements=tryout.requirements,
        test_cases=tryout.test_cases, expected_deliverables=tryout.expected_deliverables or [],
        estimated_duration_hours=tryout.estimated_duration_hours,
        payment_amount=tryout.payment_amount, payment_currency=tryout.payment_currency,
        scoring_rubric=tryout.scoring_rubric, passing_score=tryout.passing_score,
        auto_grade_enabled=tryout.auto_grade_enabled, max_submissions=tryout.max_submissions,
        submission_format=tryout.submission_format, status=tryout.status,
        submissions_count=tryout.submissions_count,
        created_at=tryout.created_at, updated_at=tryout.updated_at
    )


def submission_to_response(sub):
    return SubmissionResponse(
        id=str(sub.id), tryout_id=str(sub.tryout_id), candidate_id=str(sub.candidate_id),
        submission_url=sub.submission_url, submission_data=sub.submission_data,
        notes=sub.notes, status=sub.status, auto_score=sub.auto_score,
        manual_score=sub.manual_score, final_score=sub.final_score,
        score_breakdown=sub.score_breakdown, feedback=sub.feedback,
        reviewed_by=sub.reviewed_by, reviewed_at=sub.reviewed_at,
        payment_status=sub.payment_status, payment_escrowed_at=sub.payment_escrowed_at,
...
async def create_tryout(
    ...
    tryout = await tryout_service.create_tryout(
        db=db, employer_id=uuid.UUID(current_user["user_id"]),
...
async def get_tryout_by_job(job_id: str, db: AsyncSession = Depends(get_db)):
    ...
    if not tryout:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Tryout not found for this job")
    return tryout_to_response(tryout)
...
async def update_tryout(
    ...
    tryout = await tryout_service.update_tryout(
        db=db, tryout_id=uuid.UUID(tryout_id),
...
async def get_tryout_submissions(
    ...
    submissions, total = await submission_service.get_tryout_submissions(
        db=db, tryout_id=uuid.UUID(tryout_id),
        employer_id=uuid.UUID(current_user["user_id"]), page=page, page_size=page_size
    )
    return SubmissionListResponse(
        submissions=[submission_to_response(s) for s in submissions],
        total=total, page=page, page_size=page_size
