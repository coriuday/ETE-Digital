...
if [ ! -f "$ENV_FILE" ]; then
    ...
    # Docker Compose reads variable substitution from ete-digital/.env (not backend/.env)
    cp "$VPS_EXAMPLE" "$INSTALL_DIR/ete-digital/.env"
...
info "   5. Ensure .env exists here too (compose reads it for CADDY_DOMAIN, VITE_API_URL)"
info "   6. docker compose --profile vps up -d --build"
info "   7. curl https://yourdomain.com/health"
info "═══════════════════════════════════════════════════════════════"
