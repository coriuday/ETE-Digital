 ...
 * Shows a candidate's own applications with status, match score, and job details
 */
...
function formatSalary(min: number | null, max: number | null, currency: string) {
    ...
    const fmt = (n: number) =>
        currency === 'INR' ? `₹${(n / 100000).toFixed(1)}L` : `$${(n / 1000).toFixed(0)}K`;
                ...
                {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center h-48 text-red-500 text-sm">
                        ...
                        <button
                            ...
                            className="mt-3 px-4 py-2 bg-gray-100 rounded-lg text-sm text-gray-700 hover:bg-gray-200 transition-colors"
                        >
                            Retry
                        </button>
                    </div>
                    ...
                    <div className="flex flex-col items-center justify-center h-56 text-gray-400">
                        ...
                        {!statusFilter && (
                            <Link
                                to="/jobs"
                                className="mt-4 px-5 py-2 bg-blue-600 text-white rounded-xl text-sm font-semibold hover:bg-blue-700 transition-colors flex items-center gap-2"
                            >
                    ...
                    <div className="space-y-3">
                        {filtered.map((app) => {
                            ...
                            const salary = app.job
                                ? formatSalary(app.job.salary_min, app.job.salary_max, app.job.salary_currency)
                                : null;
                                        ...
                                        <div className="flex items-start gap-3">
                                            ...
                                            <div>
                                                ...
                                                <div className="flex flex-wrap items-center gap-2 mt-1.5">
                                                    {app.job?.job_type && (
                                                        <span className="px-2 py-0.5 bg-gray-100 rounded-md text-xs text-gray-500 capitalize">
                                                            {app.job.job_type.replace('_', ' ')}
                                                        </span>
                                                    )}
                                                    {salary && (
                                                        <span className="px-2 py-0.5 bg-emerald-50 rounded-md text-xs text-emerald-600">
                                                            {salary} /yr
                                                        </span>
                                                    )}
                                                </div>
                                            ...
                                            {app.match_score !== null && (
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <div className="w-16 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                                                        <div
                                                            className={`h-full rounded-full ${app.match_score >= 80 ? 'bg-emerald-500'
                                                                    : app.match_score >= 60 ? 'bg-amber-500'
                                                                        : 'bg-red-400'
                                                                }`}
