...
import {
    ...
    LayoutGrid, List, ArrowRight, Building2, Clock, Banknote, X, Zap, ShieldCheck
} from 'lucide-react';
        ...
        <aside className="w-full rounded-3xl p-6 space-y-8 relative overflow-hidden bg-white border border-gray-200 shadow-sm">
            ...
            <div className="flex items-center justify-between p-4 rounded-xl bg-primary-50 border border-primary-200">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-primary-500">Skill-based hiring</p>
                    </div>
                </div>
                <label className="relative inline-flex items-center cursor-pointer">
                    ...
                    />
                    <div className="w-11 h-6 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-primary-500 transition-colors bg-gray-200 border border-gray-300"></div>
                </label>
            </div>
                        ...
                        <label key={type} className="flex items-center gap-3 cursor-pointer group">
                            <div className="relative flex items-center justify-center">
                                <input
                                    ...
                                    className="peer appearance-none w-5 h-5 rounded border transition-colors cursor-pointer border-gray-300 bg-white checked:bg-primary-500 checked:border-primary-500"
                                />
            ...
            <div>
                ...
                <div className="relative group">
                    <Banknote size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-primary-500" />
                    <input
                        type="number"
                        placeholder="e.g. 100000"
                        value={filters.salaryMin}
                        onChange={(e) => onChange({ ...filters, salaryMin: e.target.value })}
                        className="w-full pl-9 pr-4 py-3 rounded-xl text-sm outline-none focus:ring-2 transition-all bg-gray-50 border border-gray-200 text-gray-900 focus:ring-primary-500/30 focus:border-primary-400 placeholder:text-gray-400"
                    />
                </div>
            </div>
...
function JobCard({ job, view }: { job: Job; view: 'grid' | 'list' }) {
    ...
    return (
        <motion.div
            whileHover={{ y: -4 }}
            whileTap={{ scale: 0.98 }}
            className={`group relative rounded-2xl transition-all p-6 flex cursor-pointer bg-white border border-gray-100 hover:border-primary-300 hover:shadow-lg shadow-sm
        ...
        >
            ...
            <div className="flex w-full gap-4 items-start">
                <div className="flex-shrink-0">
                    <div className="w-12 h-12 rounded-xl flex items-center justify-center bg-primary-50 border border-primary-100 group-hover:border-primary-300">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-2 mb-4">
                        {job.has_tryout && (
                            <span className="flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold tracking-wide bg-primary-50 border border-primary-200 text-primary-700">
                    ...
                    <div className="hidden sm:flex flex-col items-end flex-shrink-0 relative z-20">
                        {job.salary_min != null || job.salary_max != null ? (
                            <div className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-500 to-emerald-600 font-bold text-lg text-right">
                                {salaryLabelOrDefault(job.salary_min, job.salary_max, job.salary_currency)}
                            </div>
                        ) : (
                            <div className="font-medium text-sm text-right text-gray-400">Competitive salary</div>
                        )}
                        <span className="mt-4 px-5 py-2 rounded-full text-sm font-bold border border-primary-300 text-primary-600 group-hover:bg-primary-600 group-hover:text-white transition-all pointer-events-none">
                            View Details
                        </span>
                    </div>
                )}
            ...
            <div className={`w-full ${isGrid ? 'mt-4 pt-4' : 'sm:hidden mt-4 pt-4'} border-t border-gray-100`}>
                ...
                <div className="flex items-center justify-between mt-auto">
                    ...
                    <span className="flex items-center gap-1 text-sm font-bold group-hover:gap-2 transition-all pointer-events-none text-primary-600">
            ...
            <div className="relative z-10 pt-24 pb-8 border-b border-gray-200 bg-white/80 backdrop-blur-md">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <form onSubmit={handleSearch}
                        ...
                        <div className="flex-1 flex items-center gap-3 px-4 rounded-2xl border bg-gray-100 border-gray-200 focus-within:border-primary-400 focus-within:ring-1 focus-within:ring-primary-400 transition-all">
                        ...
                        <div className="md:w-1/3 flex items-center gap-3 px-4 rounded-2xl border bg-gray-100 border-gray-200 focus-within:border-primary-400 focus-within:ring-1 focus-within:ring-primary-400 transition-all">
                        ...
                        <button type="submit"
                            className="flex items-center justify-center gap-2 px-8 py-4 bg-gradient-to-r from-primary-600 to-primary-600 text-white font-bold rounded-2xl hover:from-primary-700 hover:to-primary-700 active:scale-95 transition-all shadow-[0_0_20px_rgba(2,94,187,0.2)]">
                            Find Jobs
                        </button>
                    </form>
                </div>
                        ...
                        <div className="flex items-center justify-between mb-8 p-4 rounded-2xl border bg-white border-gray-200">
                            ...
                            <div className="flex items-center gap-1 border rounded-xl p-1 bg-gray-100 border-gray-200">
                                <button onClick={() => setView('grid')}
                                    className={`p-2 rounded-lg transition-all active:scale-90 ${view === 'grid' ? 'bg-white text-primary-600 shadow-sm' : 'text-gray-400 hover:text-gray-700'}`}>
                                    <LayoutGrid size={18} />
                                </button>
                                <button onClick={() => setView('list')}
                                    className={`p-2 rounded-lg transition-all active:scale-90 ${view === 'list' ? 'bg-white text-primary-600 shadow-sm' : 'text-gray-400 hover:text-gray-700'}`}>
                                    <List size={18} />
                                </button>
                            </div>
                        </div>
