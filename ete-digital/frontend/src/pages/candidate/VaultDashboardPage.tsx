    ...
    if (view === 'list') {
        return (
            <div className="bg-white rounded-2xl border border-gray-100 hover:border-blue-200 hover:shadow-md transition-all p-5 flex items-center gap-4">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 border ${cfg.bg} ${cfg.color}`}>{cfg.icon}</div>
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                        <h3 className="font-bold text-gray-900 truncate">{item.title}</h3>
                        {item.is_verified && <span className="flex-shrink-0 flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs font-bold"><Shield size={10} /> Verified</span>}
                    </div>
                ...
                <div className="flex items-center gap-1.5">
                    <Link to={`/vault/edit/${item.id}`} className="p-2 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-blue-600 transition-colors"><Edit size={15} /></Link>
                    <button onClick={() => onDelete(item.id)} className="p-2 rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-600 transition-colors"><Trash2 size={15} /></button>
                </div>
            </div>
        );
    }
        ...
        <div className="group bg-white rounded-2xl border border-gray-100 hover:border-blue-200 hover:shadow-lg transition-all overflow-hidden">
            ...
            <div className="p-5">
                <div className="flex items-start justify-between gap-2 mb-3">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center border ${cfg.bg} ${cfg.color}`}>{cfg.icon}</div>
                    <div className="flex items-center gap-1">
                        {item.is_verified && <span className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs font-bold"><Shield size={10} /> Verified</span>}
                        <span className={`px-2 py-0.5 rounded-full border text-xs font-semibold ${cfg.bg} ${cfg.color}`}>{cfg.label}</span>
                    </div>
                </div>
                ...
                <div className="flex items-center justify-between pt-3 border-t border-gray-50">
                    ...
                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <Link to={`/vault/edit/${item.id}`} className="p-1.5 rounded-lg hover:bg-blue-50 text-gray-400 hover:text-blue-600 transition-colors"><Edit size={13} /></Link>
                        <button onClick={() => onDelete(item.id)} className="p-1.5 rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-600 transition-colors"><Trash2 size={13} /></button>
                    </div>
                </div>
            </div>
        </div>
...
export default function VaultDashboardPage() {
    ...
    const handleDelete = async (id: string) => {
        ...
        try { await vaultApi.deleteVaultItem(id); setItems(prev => prev.filter(i => i.id !== id)); }
        catch { alert('Failed to delete'); }
    };

    const filtered = items.filter(i => activeType === 'all' || i.type === activeType)
            ...
            <div className="bg-gradient-to-br from-slate-900 via-violet-950 to-blue-900 relative overflow-hidden">
                ...
                <div className="relative max-w-7xl mx-auto px-6 py-10">
                    <div className="flex items-center justify-between mb-6">
                        ...
                        <Link to="/vault/add" className="flex items-center gap-2 px-5 py-2.5 bg-white text-blue-700 font-bold rounded-xl hover:bg-blue-50 transition-colors text-sm shadow-lg">
            ...
            <div className="max-w-7xl mx-auto px-6 py-10">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                        {CATEGORIES.map(cat => (
                            <button key={cat} onClick={() => setActiveType(cat)}
                                className={`px-3 py-1.5 rounded-xl text-xs font-semibold capitalize transition-colors ${activeType === cat ? 'bg-blue-600 text-white shadow' : 'bg-white border border-gray-200 text-gray-600 hover:border-blue-300'}`}>
                                {cat === 'all' ? 'All' : TYPE_CONFIG[cat]?.label ?? cat}
                            </button>
                        ))}
                    </div>
                    <div className="flex items-center gap-1 bg-white border border-gray-200 rounded-xl p-1">
                        <button onClick={() => setView('grid')} className={`p-2 rounded-lg transition-colors ${view === 'grid' ? 'bg-blue-600 text-white' : 'text-gray-500 hover:bg-gray-100'}`}><Grid3x3 size={14} /></button>
                        <button onClick={() => setView('list')} className={`p-2 rounded-lg transition-colors ${view === 'list' ? 'bg-blue-600 text-white' : 'text-gray-500 hover:bg-gray-100'}`}><List size={14} /></button>
                    </div>
                </div>

                {loading ? (
                    <div className="flex items-center justify-center py-20"><Loader2 size={36} className="animate-spin text-blue-600" /></div>
                ) : filtered.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-20">
                        <Archive size={48} className="text-gray-200 mb-4" />
                        <p className="font-bold text-gray-600 mb-1">{items.length === 0 ? 'Your vault is empty' : 'No items match your filters'}</p>
                        {items.length === 0 && (
                            <Link to="/vault/add" className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-blue-600 to-violet-600 text-white font-bold rounded-xl text-sm mt-4">
                    ...
                    <div className={view === 'grid' ? 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5' : 'space-y-3'}>
                        {filtered.map(item => <VaultCard key={item.id} item={item} view={view} onDelete={handleDelete} />)}
                    </div>
                )}
                ...
                {items.length > 0 && (
                    <div className="mt-10 bg-gradient-to-r from-blue-600 to-violet-600 rounded-2xl p-6 flex flex-wrap items-center justify-between gap-4">
                        ...
                        <Link to="/vault/shares" className="flex items-center gap-2 px-5 py-2.5 bg-white text-blue-700 font-bold rounded-xl text-sm hover:bg-blue-50 transition-colors">
