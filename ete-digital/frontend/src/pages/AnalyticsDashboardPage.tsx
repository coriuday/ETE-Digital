...
export default function AnalyticsDashboardPage() {
    ...
    useEffect(() => {
        (async () => {
            ...
            try {
                ...
                setData(null); // will use mock data
            } finally {
                setLoading(false);
            }
        })();
    ...
    const lineData = data?.applications_over_time ?? mockLineData;
    const barData = data?.top_jobs ?? mockBarData;
        ...
        <AppShell>
            <div className="p-6 lg:p-8 space-y-8">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    <KpiCard label="Total Views" value={data?.total_views ?? '1,240'} icon={<BarChart2 size={18} className="text-blue-600" />} color="bg-blue-50" trend="+18%" />
                    <KpiCard label="Applications" value={data?.total_applications ?? '361'} icon={<Users size={18} className="text-violet-600" />} color="bg-violet-50" trend="+12%" />
                    <KpiCard label="Active Jobs" value={data?.active_jobs ?? '8'} icon={<Briefcase size={18} className="text-amber-600" />} color="bg-amber-50" trend="+2" />
                    <KpiCard label="Hired" value={data?.total_hired ?? '18'} icon={<Star size={18} className="text-emerald-600" />} color="bg-emerald-50" trend="+3" />
                </div>
