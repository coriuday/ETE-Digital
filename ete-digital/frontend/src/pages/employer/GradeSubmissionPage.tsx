...
export default function GradeSubmissionPage() {
    ...
    const handleSubmit = async () => {
        if (!decision) { alert('Please select pass or fail'); return; }
        setLoading(true);
        try {
            ...
            <div className="bg-white shadow">
                <div className="container mx-auto px-4 py-6">
                    <div className="flex justify-between items-center">
                        ...
                        <button onClick={() => navigate(-1)} className="px-4 py-2 text-gray-700 hover:bg-gray-100 rounded-lg transition">Back</button>
                    </div>
                </div>
            </div>
                ...
                <div className="max-w-4xl mx-auto space-y-6">
                    ...
                    <div className="bg-white rounded-lg shadow p-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div><label className="text-sm font-medium text-gray-600">Candidate</label><p className="text-gray-900 font-medium">John Doe</p></div>
                            <div><label className="text-sm font-medium text-gray-600">Tryout</label><p className="text-gray-900">Build React Dashboard</p></div>
                            <div><label className="text-sm font-medium text-gray-600">GitHub Repository</label><a href="#" className="text-primary-600 hover:text-primary-700 font-medium">View Code →</a></div>
                            <div><label className="text-sm font-medium text-gray-600">Live Demo</label><a href="#" className="text-primary-600 hover:text-primary-700 font-medium">View Demo →</a></div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-lg shadow p-6">
                        ...
                        <p className="text-gray-700 whitespace-pre-wrap">[Candidate's submission notes would appear here]</p>
                    </div>
                    ...
                    <div className="bg-white rounded-lg shadow p-6">
                        <div className="flex justify-between items-center mb-4">
                            ...
                            <div className="text-2xl font-bold text-primary-600">{totalScore} / 100</div>
                        </div>
                                ...
                                <div key={index} className="border-b border-gray-200 pb-4 last:border-0">
                                    ...
                                    <div className="flex items-center gap-4">
                                        <input type="range" min="0" max={item.maxPoints} value={scores[index]}
                                            onChange={(e) => { const n = [...scores]; n[index] = parseInt(e.target.value); setScores(n); }}
                                            className="flex-1 h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer" />
                                        <input type="number" min="0" max={item.maxPoints} value={scores[index]}
                                            onChange={(e) => { const n = [...scores]; n[index] = Math.min(item.maxPoints, Math.max(0, parseInt(e.target.value) || 0)); setScores(n); }}
                    ...
                    <div className="bg-white rounded-lg shadow p-6">
                        ...
                        <div className="flex gap-4">
                            <button onClick={() => setDecision('pass')}
                                className={`flex-1 px-6 py-4 rounded-lg font-medium transition ${decision === 'pass' ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                                ✓ Pass & Approve Payment
                            </button>
                            <button onClick={() => setDecision('fail')}
                                className={`flex-1 px-6 py-4 rounded-lg font-medium transition ${decision === 'fail' ? 'bg-red-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                                ✗ Fail & Reject Payment
                            </button>
                        </div>
                        {decision && (
                            <div className={`mt-4 p-4 rounded-lg ${decision === 'pass' ? 'bg-green-50 text-green-800' : 'bg-red-50 text-red-800'}`}>
                                {decision === 'pass' ? '✓ This candidate will receive payment and advance in the hiring process' : '✗ This candidate will not receive payment and will be notified'}
                            </div>
                        )}
                    </div>
                    ...
                    <div className="flex justify-end gap-4">
                        <button type="button" onClick={() => navigate(-1)}
                            className="px-6 py-3 text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition font-medium">Cancel</button>
