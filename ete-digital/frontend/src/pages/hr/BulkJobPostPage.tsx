            ...
            <div className="p-6 lg:p-8 max-w-5xl mx-auto space-y-6">
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-xl font-bold text-gray-900">Bulk Job Post</h1>
                            <p className="text-sm text-gray-500">Upload a CSV to create up to 200 jobs at once</p>
                        </div>
                    </div>
                    ...
                    <div className={`rounded-2xl border p-6 ${result.created > 0 ? 'bg-emerald-50 border-emerald-200' : 'bg-amber-50 border-amber-200'}`}>
                        <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1">
                                <h2 className="font-bold text-gray-900 mb-1">Upload Complete</h2>
                                ...
                                {result.errors.length > 0 && (
                                    <div className="mt-3 space-y-1">
                                        <p className="text-xs font-semibold text-gray-600 uppercase tracking-wider mb-2">Row Errors:</p>
                                        {result.errors.map(e => (
                                            <div key={e.row} className="flex items-center gap-2 text-xs bg-white rounded-lg px-3 py-1.5 border border-red-100">
                ...
                {!file ? (
                    <div
                        ...
                        className={`border-2 border-dashed rounded-2xl p-16 text-center cursor-pointer transition-all ${dragging ? 'border-indigo-400 bg-indigo-50 scale-[1.01]' : 'border-gray-200 bg-gray-50 hover:border-indigo-300 hover:bg-indigo-50/50'}`}
                    >
                        <input ref={inputRef} type="file" accept=".csv" className="hidden" onChange={onInputChange} />
                        <div className="w-16 h-16 bg-white border-2 border-dashed border-gray-300 rounded-2xl flex items-center justify-center mx-auto mb-4">
                            <FileText size={28} className="text-gray-400" />
                        </div>
                        <p className="font-semibold text-gray-700 text-lg">Drop your CSV here</p>
                        <p className="text-gray-400 text-sm mt-1">or click to browse — max 200 rows</p>
                        ...
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-gray-400">{preview.length} rows parsed</p>
                                </div>
                            </div>
                            <div className="flex items-center gap-2">
                                <button onClick={resetForm} className="text-gray-400 hover:text-gray-600 p-2 rounded-lg hover:bg-gray-50 transition-colors">
                                    <X size={16} />
                                </button>
                                <button
                            ...
                            <div className="overflow-x-auto rounded-xl border border-gray-100">
                                <table className="w-full text-xs">
                                    <thead className="bg-gray-50">
                                        <tr>
                                            <th className="px-3 py-2 text-left text-gray-400 font-semibold uppercase tracking-wider">#</th>
                                            {PREVIEW_COLUMNS.map(col => (
                                                <th key={col} className="px-3 py-2 text-left text-gray-400 font-semibold uppercase tracking-wider">
                                                    {col.replace('_', ' ')}
                                                </th>
                                            ))}
                                        </tr>
                                ...
                                {preview.length > 15 && (
                                    <div className="px-4 py-2 text-center text-xs text-gray-400 bg-gray-50 border-t border-gray-100">
